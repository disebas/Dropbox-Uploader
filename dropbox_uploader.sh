...
function db_simple_upload_file
{
    ...
    if grep -q "^HTTP/1.1 200 OK" "$RESPONSE_FILE"; then
        `zenity --info --text="$FILE_SRC was Upload"`
    else
        `zenity --info --text="$FILE_SRC wasn't Upload\n
...
#Account unlink
function db_unlink
{   answer=`zenity --entry --title="Unlink" --text="Are you sure you want unlink this script from your Dropbox account? [y/n]"`
        if [[ $answer == "y" ]]; then
        rm -fr "$CONFIG_FILE"
#        echo -ne "DONE\n"
    fi
...
{
    local DIR_DST=$(normalize_path "$1")
    print "Estos son tus archivos en $DIR_DST \n\n"
...
COMMAND=`zenity --list --title="Dropbox Uploader V(11.13)" --text="Select an operation" \
     --radiolist \
     --width="480" --height="350" \
     --column="O" --column="Operation" --column="Description" \
     FALSE "upload" "Upload your files"\
     FALSE "download" "Download your files" \
     FALSE "delete" "Delete a file " \
     FALSE "move" "Move files to another folder" \
     FALSE "copy" "Make a copy of your files" \
     FALSE "mkdir" "Make a new folder" \
     FALSE "list" "Watch your files in your account"  \
     FALSE "share" "Get a link to share your files"  \
     FALSE "info" "Watch your info"  \
     FALSE "unlink" "Disconnect your account"`
ARG1=${@:$OPTIND+1:1}
ARG2=${@:$OPTIND+2:1}
...
case $COMMAND in
    ...
    upload)
        ...
        FILE_SRC=`zenity --file-selection --title="Dropbox Uploader V(11.13) - Upload"`
        FILE_DST=`zenity --entry --title="Destiny folder" --text="Folder"`
    ...
    download)
        ...
        FILE_SRC=`zenity --entry --title="Dropbox Uploader V(11.13) - Download" --text="Write root file"`
        FILE_DST=`zenity --entry --title="Download" --text="Write destiny folder"`
        #Checking FILE_SRC
        if [[ $FILE_SRC == "" ]]; then
            print "Error: invalid source file or directory"
            remove_temp_files
    ...
    share)
        ...
        FILE_DST=`zenity --entry --title="Dropbox Uploader V(11.13) - Share " --text="Write the file to share"`
    ...
    delete|remove)
        ...
        FILE_DST=`zenity --entry --title="Dropbox Uploader V(11.13) - Delete" --text="Write the file to delete:"`
    ...
    move|rename)
        ...
        FILE_SRC=`zenity --entry --title="Dropbox Uploader V(11.13) -Move" --text="Write the root/name file"`
        FILE_DST=`zenity --entry --title="Write destiny folder" --text="file"`
    ...
    copy)
        ...
        FILE_SRC=`zenity --entry --title="Dropbox Uploader V(11.13) - Copy" --text="Write file to copy"`
        FILE_DST=`zenity --entry --title="Write file copy destiny" --text="file"`
    ...
    mkdir)
        ...
        DIR_DST=`zenity --entry --title="Dropbox Uploader V(11.13) - Make Dir" --text="Enter folder name"`
    ...
    list)
        ...
        DIR_DST=`zenity --entry --title="Dropbox Uploader V(11.13) - List" --text="Enter folder name to list"`
        ...
        `zenity --info --text="$(db_list "/$DIR_DST")"`
